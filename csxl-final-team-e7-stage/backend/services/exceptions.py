...
class DuplicateMemberException(Exception):
    """DuplicateMemberException is raised when a user attempts to add a member when one already exists for given org_id and user_id"""

    def __init__(self, slug: str, user_id: int):
        super().__init__(
            f"Member with slug: {slug} and user_id: {user_id} already exists"
        )
