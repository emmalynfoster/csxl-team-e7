...
from backend.models.organization_member import NewOrganizationMember, OrganizationMember
...
cads = Organization(
    ...
    public=True,
    is_member=False,
    status=OrganizationStatus.OPEN.value,
    application_link=""
)
...
cssg = Organization(
    ...
    public=False,
    status=OrganizationStatus.CLOSED.value,
    application_link=""
)
...
appteam = Organization(
    ...
    public=False,
    status=OrganizationStatus.APPLICATION_BASED.value,
    application_link=""
)
...
organization_member1 = NewOrganizationMember(
...
organization_member2 = NewOrganizationMember(
    organization_id = cads.id | 0,
    user_id = user_org_members.id | 0,
    year = 2024,
    semester=Semester.SPRING
)

#For a user that has joined multiple organizations
organization_member3 = NewOrganizationMember(
...
members = [organization_member1, organization_member2, organization_member3]
...
def insert_fake_data(session: Session):
    ...
    for org in organizations:
        organization_entity = OrganizationEntity.from_model(org)
        session.add(organization_entity)
        entities.append(organization_entity)

    for member in members:
        member_entity = OrganizationMemberEntity.from_new_model(member)
